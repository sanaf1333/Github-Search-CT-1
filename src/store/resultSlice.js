const { createSlice, combineReducers, createAsyncThunk } = require('@reduxjs/toolkit');
import { useDebouncedCallback } from '../components/debounce';
     ...
     return async function fetchProductThunk(dispatch, getState) {
         ...
         try {
            const headers = { Accept: "application/vnd.github.text-match+json" };
             const res = await fetch('https://api.github.com/search/'+props+'&per_page=100&media=text-match', { headers });
             ...
             const usersWithMetadata = data.items.map(user => {
                ...
                const metadata = rest.text_matches ? rest.text_matches.reduce((acc, match) => {
